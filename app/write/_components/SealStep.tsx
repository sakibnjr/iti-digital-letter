  ...
  return (
    <div ref={containerRef} className="w-full max-w-2xl mx-auto px-4 py-8 flex flex-col items-center">
      ...
      <div className="w-full flex items-center justify-between mb-8">
        <button type="button" onClick={onBack} disabled={sealing} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-serif text-slate-800 hover:bg-blue-900/10 border border-blue-950/20 transition-all cursor-pointer disabled:opacity-40">
          <ArrowLeft className="w-3.5 h-3.5 text-blue-900" />
          {lang === 'bn' ? 'চিঠিতে ফিরুন' : 'Back'}
        </button>
        <h2 className="font-serif text-xl font-bold text-slate-900">{lang === 'bn' ? 'সিলমোহর' : 'Wax Seal'}</h2>
        <div className="w-16" />
      </div>
      ...
      {/* Options panel */}
      <div className="w-full max-w-lg bg-[#FAFDFE] border border-blue-900/20 rounded-2xl p-5 shadow-xs space-y-5 paper-grain">
        {/* Emblem */}
        <div>
          <label className="block text-xs font-serif font-bold text-slate-900 mb-2">{lang === 'bn' ? 'নকশা:' : 'Emblem:'}</label>
          <div className="grid grid-cols-4 sm:grid-cols-7 gap-2">
            {WAX_DESIGNS.map((d) => (
              <button key={d.id} type="button" onClick={() => setWaxDesign(d.id)} className={`p-2 rounded-xl border text-center transition-all flex flex-col items-center cursor-pointer ${waxDesign === d.id ? 'border-blue-800 bg-blue-900/10 font-bold shadow-xs' : 'border-blue-900/15 hover:border-blue-900/30 bg-white/70'}`}>
                <span className="text-base">{d.symbol}</span>
                <span className="text-[10px] font-serif text-slate-800 mt-1 truncate">{lang === 'bn' ? d.nameBn : d.nameEn}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        {/* Wax color */}
        <div>
          <label className="block text-xs font-serif font-bold text-slate-900 mb-2">{lang === 'bn' ? 'মোমের রং:' : 'Wax:'}</label>
          <div className="flex gap-2.5 overflow-x-auto py-1">
            {WAX_COLORS.map((c) => (
              <button key={c.id} type="button" onClick={() => setWaxColor(c.id)} className={`w-7 h-7 rounded-full transition-transform flex items-center justify-center shadow-xs cursor-pointer ${waxColor === c.id ? 'scale-125 ring-2 ring-blue-900 ring-offset-2' : 'hover:scale-110 opacity-90'}`} style={{ backgroundColor: c.hex }}>
        ...
        {/* Envelope color */}
        <div>
          <label className="block text-xs font-serif font-bold text-slate-900 mb-2">{lang === 'bn' ? 'খামের রং:' : 'Envelope:'}</label>
          <div className="flex gap-2.5 overflow-x-auto py-1">
            {ENVELOPE_COLORS.map((e) => (
              <button key={e.id} type="button" onClick={() => setEnvColor(e.id)} className={`w-7 h-7 rounded-lg transition-transform flex items-center justify-center shadow-xs border border-white/20 cursor-pointer ${envColor === e.id ? 'scale-125 ring-2 ring-blue-900 ring-offset-2' : 'hover:scale-110 opacity-90'}`} style={{ backgroundColor: e.id }}>
