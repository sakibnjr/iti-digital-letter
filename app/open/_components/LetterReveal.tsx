...
export default function LetterReveal({ letter, lang }: LetterRevealProps) {
  ...
  return (
    <div ref={ref} className="w-full max-w-2xl mx-auto px-4 py-8">
      {/* Parchment */}
      <div className="relative rounded-2xl p-6 sm:p-12 shadow-xl bg-[#FAFDFE] text-slate-900 deckle-edge paper-grain border border-blue-950/20" style={{ fontFamily: 'var(--font-bn-serif)' }}>
        ...
        {/* Salutation */}
        <div className="text-lg sm:text-xl font-bold font-serif mb-6 text-slate-900">{letter.salutation} {letter.recipientName},</div>

        {/* Body */}
        <div data-letter-body className="text-base sm:text-lg leading-relaxed mb-8 whitespace-pre-wrap text-slate-900" style={{ lineHeight: '2.0' }}>
          {letter.body}
        </div>

        {/* Sign-off */}
        <div data-letter-sign className="flex flex-col items-end text-right mt-8 pt-4 border-t border-blue-900/15 font-serif">
          <div className="text-base sm:text-lg font-semibold text-slate-800">{letter.signOff}</div>
          <div className="text-lg sm:text-xl font-bold text-slate-950 mt-1">{letter.senderName}</div>
        </div>

        {/* P.S. */}
        {letter.postScript && (
          <div className="mt-8 pt-4 border-t border-blue-900/15 text-xs text-slate-600 font-serif">
            <span className="font-bold text-slate-800">{lang === 'bn' ? 'পুনশ্চ: ' : 'P.S. '}</span>{letter.postScript}
          </div>
        )}
      </div>
